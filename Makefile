...
generate:
	...
	rm -rf pkg/api/restapi
	swagger generate server --exclude-main -f pkg/api/swagger.yml --template=stratoscale -t pkg/api
...
swagger.validate:
	swagger validate pkg/api/swagger.yml
...
swagger.doc:
	docker run -i yousan/swagger-yaml-to-html < pkg/api/swagger.yml > doc/index.html


GO_BUILD_ENV := CGO_ENABLED=0 GOOS=linux GOARCH=amd64
DOCKER_BUILD=$(shell pwd)/.docker_build
DOCKER_CMD=$(DOCKER_BUILD)/social-naka-app-services
